...
export default function ProcessStatesPage() {
  ...
  > = {
    NEW: {
      ...
      desc:
        'The process is being created and configured by the kernel (e.g., via fork() and exec()). The PCB is allocated, but the process is not yet admitted to the main memory ready queue.',
      hardware:
        'Kernel allocates PCB in kernel space; executable binary read from disk storage.',
    },
    ...
    READY: {
      ...
      desc:
        'The process resides in main RAM and is waiting to be allocated CPU execution time by the kernel scheduler. All prerequisite resources (I/O, memory, locks) are ready.',
      hardware:
        'Resident in RAM; linked into the Scheduler Ready Queue linked-list.',
    },
    ...
    RUNNING: {
      ...
      desc:
        'Instructions are actively executing on physical CPU arithmetic logic units (ALU). Program counter is incrementing, registers are updating.',
      hardware:
        'Physical CPU core registers loaded with PCB context; MMU translating active virtual addresses.',
    },
    ...
    WAITING: {
      ...
      desc:
        'The process cannot run even if CPU cores are idle because it is waiting for an external event to occur (e.g. disk read completion, network packet, timer, or mutex release).',
      hardware:
        'CPU freed for other processes; process context linked into Device Wait Queue.',
    },
    ...
    TERMINATED: {
      ...
      desc:
        'The process has completed execution (via exit() or fatal signal). Memory space is deallocated, but the PCB remains as a zombie until the parent collects its exit status via wait().',
      hardware:
        'Memory page tables freed; exit code preserved in PCB for parent process retrieval.',
    },
  };
    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-border pb-6">
        ...
        <p className="text-sm text-muted mt-2 leading-relaxed max-w-3xl">
          During its execution lifetime, a process cycles through
          distinct operating system states. The CPU scheduler
          continuously mediates transitions between these states in
          response to hardware clock interrupts, system calls, and
          I/O events.
        </p>
      </div>
      ...
      <div className="p-6 rounded-xl bg-surface-alt border border-border space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted mt-0.5">
              Trigger kernel transitions and observe how the process
              lifecycle state machine responds.
            </p>
          </div>

          <button
            onClick={resetProcess}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface border border-border text-xs text-muted hover:text-foreground hover:bg-surface transition-colors"
          >
            <RotateCcw size={13} />
            Reset Process
          </button>
            ...
            {(
              ...
              return (
                <motion.div
                  ...
                  className={`p-3 rounded-lg border-2 flex flex-col items-center justify-center min-h-[90px] relative transition-all ${isCurrent
                    ? `${info.bg} shadow-md shadow-accent/10 font-bold`
                    : 'bg-surface-alt/50 border-border/60 opacity-60'
                    }`}
                >
                  {isCurrent && (
                    <span className="absolute -top-2.5 px-2 py-0.5 rounded-full bg-accent text-white text-[9px] font-mono uppercase tracking-wider">
                      Active State
                    </span>
                  )}

                  <span
                    className={`text-xs font-mono font-bold ${info.color}`}
                  >
                    {status}
                  </span>
          ...
          <div className="mt-6 p-4 rounded-lg bg-surface-alt border border-border space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-muted font-mono">
                Status:{' '}
                {STATE_DESCRIPTIONS[currentState].title}
              </span>
            </div>
            ...
            <div className="text-[11px] text-foreground/80 font-mono pt-1">
              ...
              <span className="text-muted">
                {STATE_DESCRIPTIONS[currentState].hardware}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <span className="text-xs font-semibold text-foreground block">
            Available Kernel Event Triggers from current state (
            {currentState}):
          </span>
            ...
            {currentState === 'NEW' && (
              <button
                onClick={() =>
                  transitionTo(
                    'READY',
                    'sys_admit() — memory allocated, admitted to ready queue'
                  )
                }
            ...
            {currentState === 'READY' && (
              <button
                onClick={() =>
                  transitionTo(
                    'RUNNING',
                    'Scheduler Dispatcher selects process for CPU execution'
                  )
                }
              ...
              <>
                ...
                <button
                  onClick={() =>
                    transitionTo(
                      'WAITING',
                      'Process issues blocking I/O system call (e.g. read())'
                    )
                  }
                ...
                <button
                  onClick={() =>
                    transitionTo(
                      'TERMINATED',
                      'Process calls exit(0) or terminates execution'
                    )
                  }
            ...
            {currentState === 'WAITING' && (
              <button
                onClick={() =>
                  transitionTo(
                    'READY',
                    'Hardware disk controller raises interrupt: I/O operation complete'
                  )
                }
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 text-xs font-medium transition-colors"
              >
      ...
      <div className="p-6 rounded-xl bg-surface-alt border border-border space-y-4">
        ...
        <p className="text-xs text-muted leading-relaxed">
          In real operating systems, physical RAM is limited. When
          main memory becomes overcommitted, the kernel memory manager
          (swapper/pager) suspends processes by swapping their memory
          footprint onto disk storage (swap space). This adds two
          suspended states:
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="p-4 rounded-lg bg-surface border border-border space-y-2">
            ...
            <p className="text-muted">
              The process is waiting for an I/O event AND has been
              swapped out to secondary disk storage. When its I/O event
              finishes, it transitions to{' '}
              <strong>Ready-Suspended</strong>.
            </p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-surface border border-border space-y-2">
            ...
            <p className="text-muted">
              The process is ready to run as soon as it is swapped back
              into main RAM. When memory space opens up, the
              medium-term scheduler activates it to the regular{' '}
              <strong>Ready</strong> queue.
            </p>
          </div>
        </div>
      </div>
