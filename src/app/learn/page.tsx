...
const MODULES = [
  {
    ...
    features: ['Process Control Block (PCB)', 'Address Space (Stack, Heap, Data, Text)', 'User vs Kernel Threads', 'Context Switching Overhead'],
    icon: Layers,
    color: 'text-blue-500',
  ...
  },
  {
    ...
    features: ['5-State Process Model', '7-State Swapping & Suspension', 'Interactive State Machine Controller', 'Preemption & I/O Events'],
    icon: Activity,
  ...
  },
  {
    ...
    features: ['CPU-I/O Burst Cycles', 'Preemptive vs Non-Preemptive', 'Turnaround, Waiting, and Response Times', 'Throughput & CPU Utilization'],
    icon: Cpu,
  ...
  },
  {
    ...
    features: ['FCFS & Convoy Effect', 'SJF & SRTF Optimality Proofs', 'Priority Inversion & Aging', 'Round Robin Quantum Tuning'],
    icon: GitBranch,
    ...
    <div className="space-y-8">
      ...
      <div className="p-6 rounded-2xl bg-surface-alt border border-border space-y-3">
        ...
        <p className="text-sm text-muted leading-relaxed max-w-2xl">
          Welcome to the SchedLab learning suite. Operating systems allocate scarce hardware resources (CPU cores, memory, I/O bandwidth) across competing processes. Follow the modular curriculum below to build deep mental models of concurrency, process lifecycles, and scheduling mechanics.
        </p>
      </div>
        ...
        {MODULES.map((m, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`group p-5 rounded-xl bg-surface-alt border border-border flex flex-col justify-between transition-all ${m.borderColor}`}
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className={`p-2 rounded-lg bg-surface border border-border ${m.color}`}>
                    <Icon size={20} />
                  </div>
      ...
      <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 text-slate-300 font-mono text-xs space-y-3">
        ...
        <div className="space-y-1.5 text-[11px]">
          <p className="text-emerald-400">$ schedlab --explain-goal</p>
          <p className="text-slate-400">
            &quot;The core goal of CPU scheduling is to maximize CPU utilization and throughput while minimizing turnaround time, waiting time, and response time.&quot;
          </p>
          <p className="text-emerald-400 mt-2">$ schedlab --next-step</p>
          <p className="text-slate-400">
            Ready to explore? Select <strong>Processes & Threads</strong> from the menu or launch the simulator directly.
          </p>
        </div>
      </div>
    </div>
