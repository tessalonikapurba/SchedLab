...
export default function ComparePage() {
  ...
  const {
    ...
    setAlgorithm,
    setQuantum,
    setPriorityDirection: setSimPriority,
    setAgingInterval,
    initializeSimulation,
  } = useSimulationStore();

  const [activeMetric, setActiveMetric] = useState<
    'waiting' | 'turnaround' | 'response' | 'switches' | 'fairness' | 'queue'
  >('waiting');
  ...
  const bestWaiting = entries.length
    ? entries.reduce((min, e) =>
      e.result.metrics.avgWaitingTime <
        min.result.metrics.avgWaitingTime
        ? e
        : min
    )
    : null;
  ...
  const bestTurnaround = entries.length
    ? entries.reduce((min, e) =>
      e.result.metrics.avgTurnaroundTime <
        min.result.metrics.avgTurnaroundTime
        ? e
        : min
    )
    : null;
  ...
  const bestResponse = entries.length
    ? entries.reduce((min, e) =>
      e.result.metrics.avgResponseTime <
        min.result.metrics.avgResponseTime
        ? e
        : min
    )
    : null;
  ...
  const bestFairness = entries.length
    ? entries.reduce((max, e) =>
      e.result.metrics.fairnessIndex >
        max.result.metrics.fairnessIndex
        ? e
        : max
    )
    : null;
  ...
  const lowestSwitches = entries.length
    ? entries.reduce((min, e) =>
      e.result.metrics.contextSwitches <
        min.result.metrics.contextSwitches
        ? e
        : min
    )
    : null;
  ...
  const maxWait = Math.max(
    ...entries.map((e) => e.result.metrics.avgWaitingTime),
    1
  );

  const maxTAT = Math.max(
    ...entries.map((e) => e.result.metrics.avgTurnaroundTime),
    1
  );

  const maxResp = Math.max(
    ...entries.map((e) => e.result.metrics.avgResponseTime),
    1
  );

  const maxSwitches = Math.max(
    ...entries.map((e) => e.result.metrics.contextSwitches),
    1
  );

  const maxQueue = Math.max(
    ...entries.map((e) => e.result.metrics.avgQueueLength),
    1
  );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-muted">
                {entries.length || 7} Algorithms Evaluated
              </span>
            </div>
            ...
            <p className="text-sm text-muted mt-1">
              Benchmark FCFS, SJF, SRTF, Priority, Round Robin, MLFQ, and
              Priority with Aging on identical workload inputs under
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {PRESET_SCENARIOS.slice(0, 5).map((p) => (
              <button
                ...
                className="px-2.5 py-1 text-xs rounded border border-border bg-surface-alt hover:bg-surface text-foreground transition-colors"
              >
                {p.name.split(' (')[0]}
              </button>
            ))}
        ...
        <div className="p-4 rounded-xl bg-surface-alt border border-accent/40 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="shrink-0 flex items-center gap-2">
            <button
              onClick={() =>
                handleRunInSimulator(recommendation.primaryAlgorithm)
              }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <div className="text-base font-bold text-foreground">
                    ...
                    <span className="font-mono text-blue-500">
                      {bestTurnaround.result.metrics.avgTurnaroundTime.toFixed(
                        2
                      )}
                      u
                    </span>
                  ...
                  <p className="text-[10px] text-muted mt-0.5">
                    Optimal overall completion speed.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-base font-bold text-foreground">
                    {bestFairness.algorithm} —{' '}
                    <span className="font-mono text-emerald-500">
                      {bestFairness.result.metrics.fairnessIndex.toFixed(3)}
                    </span>
                  </div>

                  <p className="text-[10px] text-muted mt-0.5">
                    Best anti-starvation balance.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/30 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-base font-bold text-foreground">
                    {lowestSwitches?.algorithm} —{' '}
                    <span className="font-mono text-purple-500">
                      {lowestSwitches?.result.metrics.contextSwitches}{' '}
                      switches
                    </span>
                  </div>

                  <p className="text-[10px] text-muted mt-0.5">
                    Minimal CPU context swaps.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="p-5 rounded-xl bg-surface-alt border border-border space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted">
              {entries.length} algorithms evaluated
            </span>
          </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-border bg-surface text-muted">
                  ...
                  <th className="py-3 px-4 font-semibold font-mono text-amber-500">
                    Avg Waiting (WT)
                  </th>
                  <th className="py-3 px-4 font-semibold font-mono text-blue-500">
                    Avg Turnaround (TAT)
                  </th>
                  <th className="py-3 px-4 font-semibold font-mono text-teal-500">
                    Avg Response (RT)
                  </th>
                  <th className="py-3 px-4 font-semibold font-mono text-purple-500">
                    Context Switches
                  </th>
                  <th className="py-3 px-4 font-semibold font-mono text-emerald-500">
                    Fairness Index
                  </th>
                  <th className="py-3 px-4 font-semibold font-mono text-red-500">
                    Starvation Risk
                  </th>
              ...
              <tbody className="divide-y divide-border">
                {entries.map(({ algorithm, result }) => {
                  const isMinWait =
                    bestWaiting &&
                    result.metrics.avgWaitingTime ===
                    bestWaiting.result.metrics.avgWaitingTime;
                  ...
                  const isMinResp =
                    ...
                    bestResponse.result.metrics.avgResponseTime;

                  const isMaxFairness =
                    bestFairness &&
                    result.metrics.fairnessIndex ===
                    bestFairness.result.metrics.fairnessIndex;
                    ...
                    >
                      <td className="py-3 px-4">
                        ...
                        <span className="text-[11px] text-muted">
                          {ALGORITHM_METADATA[algorithm]?.preemptive
                            ? 'Preemptive'
                            : 'Non-preemptive'}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${result.metrics.starvationRisk === 'None'
            ...
            <div className="flex flex-wrap items-center gap-1.5 p-1 bg-surface rounded-lg border border-border">
              {(
                ...
                <button
                  ...
                  className={`px-2.5 py-1 text-xs font-medium rounded-md transition-colors ${activeMetric === tab.id
